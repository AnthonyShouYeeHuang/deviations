...
function fromDateId(id: string): Date | null {
...
function addDays(id: string, delta: number): string {
...
type Source = "url" | "storage" | "system";

function initial(): { id: string; source: Source } {
  const url = new URL(window.location.href);
  const qs = url.searchParams.get("devDay");
  if (qs && fromDateId(qs)) return { id: qs, source: "url" };
  const saved = localStorage.getItem(KEY);
  if (saved && fromDateId(saved)) return { id: saved, source: "storage" };
  return { id: toDateId(new Date()), source: "system" };
}

export function useDevDay() {
  const init = initial();
  const [dayId, setDayIdState] = React.useState<string>(init.id);
  const [source, setSource] = React.useState<Source>(init.source);

  function setDayId(next: string) {
