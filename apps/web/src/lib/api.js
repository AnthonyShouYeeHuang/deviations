...
export async function getToday() {
    ...
    if (!r.ok)
        throw new Error(`/api/today ${r.status}`);
    return r.json();
}
export async function getProblem(day) {
...
export async function postAnswer(body) {
    ...
    if (!r.ok)
        throw new Error(`/api/answer ${r.status}`);
    return r.json();
}
export async function getResults(day) {
    const r = await fetch(apiUrl(`/api/results?day=${encodeURIComponent(day)}`), { cache: "no-store" });
    if (!r.ok)
        throw new Error(`/api/results ${r.status}`);
    return r.json();
}
