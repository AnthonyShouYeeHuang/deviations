export function evaluate(problem, yourAnswer, aggregates) {
    switch (problem.winType) {
        ...
        case "most_common_choice": {
            ...
            const mine = typeof yourAnswer === "object" && yourAnswer && "optionId" in yourAnswer
                ? yourAnswer.optionId
                : undefined;
        ...
        case "within_sd_below_mean": {
            ...
            const std = aggregates.stddev;
            const val = typeof yourAnswer === "number" ? yourAnswer : Number.NaN;
            // Rule: val must be < mean AND >= mean - stddev
            const ok = typeof mean === "number" &&
                typeof std === "number" &&
                Number.isFinite(val) &&
                val < mean &&
                val >= mean - std;
        ...
        case "closest_to_pct_of_mean": {
            ...
            const inRange = (typeof cfg.min !== "number" || val >= cfg.min) &&
